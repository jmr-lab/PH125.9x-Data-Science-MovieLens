...
#####################################################
###         The following lines will allow        ###
###         us to download the cover image        ###
###         and store it locally                  ###
#####################################################

# URL of the movielens cover image
url <- "https://raw.githubusercontent.com/jmr-lab/PH125.9x-Data-Science-MovieLens/refs/heads/main/movielens-cover.png"

# Destination file name
destfile <- "movielens-cover.png"

# Download the file
tryCatch({
  download.file(url, destfile, mode = "wb")
}, error = function(e) {
  # If the download fails, create a 1px blank image
  blank_image <- png::writePNG(array(1, dim = c(1, 1, 3)), destfile)
})
...
edx_movies <- edx %>%
  mutate(
    ...
    t_day_of_week = ((wday(datetime) + 5) %% 7) + 1,
    t_day = day(datetime),
    t_month = month(datetime),
    t_year = year(datetime),
    t_hour = hour(datetime),
    year = as.integer(str_extract(title, "(?<=\\()\\d{4}(?=\\))")),
    title_without_year = str_remove(title, "\\s*\\(\\d{4}\\)")
  ) %>%
...
# will be too long (approximately 1,240s on my lapop)
edx_movies_dt <- as.data.table(edx_movies)[, .(userId, movieId, rating)]

# Define the get_rmse function
get_rmse <- function(lambda) {
  ...
  predictions <- mu + edx_movies_dt$b_m_reg + edx_movies_dt$b_mu_reg
  
  # Remove the columns b_m_reg and b_mu_reg
  edx_movies_dt[, c("b_m_reg", "b_mu_reg") := NULL]
