...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{title-image.jpg}
  \label{fig:my_image}
\end{figure}
...
kable(data_summary, align = 'l') %>% kable_styling(full_width = F)
```
...
```{r, echo=FALSE}
kable(correlation_matrix) %>% kable_styling(full_width = F, font_size = 8)
```
...
```{r, echo=FALSE}
kable(unique_rows_2d) %>% kable_styling(full_width = F)
```
...
```{r, echo=FALSE}
kable(unique_rows_3d) %>% kable_styling(full_width = F)
```
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Number of Ratings per Movie per year of release and number of ratings per age of movie", fig.height=2}
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Number of ratings / Genre", fig.width=8, fig.height=4}
list_genres %>%
  arrange(desc(count)) %>%
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Number of ratings / Genre", fig.height=3}
plot_grid(
  edx_movies %>%
    group_by(genres) %>%
    summarize(
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Number of ratings per Genre and Year", fig.height=4}
edx_movies %>%
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Number of ratings per date (timestamp)", fig.height=5}
plot_grid(
  edx_movies %>%
...
The goal is to utilize a couple of variables to calculate the bias. In this example, the rating is displayed based on the movieId and the year, specifically for the user who provided the highest number of ratings :

```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Rating per user, year (timestamp) and movie", fig.height=5}
...
```{r, echo=FALSE, fig.align="center", fig.pos="H", fig.cap = "Comparison of the Distribution of ratings", fig.height=5}
...
desired_order <- c("Target", "Average", "Movie", "movieId, t_year", 
                   "User", "userId, t_year", "Genres", "t_year, year, genres")
